...
const MainContent = () => {
    ...
    // Fetch metrics data using the filters, role, and username
    const { averageAbandonmentRate, averageAbandonTime, averageQueueAnswerTime, averageAnswerTime, ServiceLevel, averageContactDuration, contactsHandeled, contactFlowTime, agentOccupancy } = FetchMetrics(filters, authContext.role, authContext.username);
    ...
    return (
        <div className="grid w-full h-full grid-cols-12 grid-rows-6 gap-4 p-2 pt-5 overflow-y-auto">
            <div className="col-span-3 row-span-2 card bg-tertiary dark:bg-primary" data-tooltip-id="tooltipAbandonmentRate" data-tooltip-content="Abandonment Rate is the percentage of calls that are abandoned by the caller before being answered by an agent. High abandonment rates can be a sign of poor customer service or long wait times. A low abandonment rate is a good indicator of customer satisfaction and efficient call handling by agents" data-tooltip-place="right">
            ...
            <div className="col-span-3 row-span-2 shadow-lg card bg-tertiary dark:bg-primary" data-tooltip-id="my-tooltipAgentOccupancy" data-tooltip-content="Agent Occupancy is the percentage of time agents are actively engaged in customer interactions in relation to their available or idle time. As a statistic, it's used to calculate call center productivity." data-tooltip-place="right">
                <div className="flex flex-row items-center justify-between">
                    <div className="px-4 py-4 bg-gray-300 rounded-xl bg-opacity-30"></div>
                    <div className="inline-flex text-sm text-gray-600 sm:text-base dark:text-white">12%</div>
                </div>
            ...
            <div className="col-span-3 row-span-2 shadow-lg card bg-tertiary dark:bg-primary" data-tooltip-id="tooltipAverageQueueAnswerTime" data-tooltip-content=" ASA is a more generalized metric that measures the average time it takes for all calls to be answered across the entire call center, regardless of the queue they are in. It provides an overall picture of the call center's efficiency in handling incoming calls. A low ASA indicates that calls are being answered quickly, while a high ASA indicates that callers are waiting a long time to speak with an agent." data-tooltip-place="right">
                <div className="flex flex-row items-center justify-between">
                    <div className="px-4 py-4 bg-gray-300 rounded-xl bg-opacity-30"></div>
                    <div className="inline-flex text-sm text-gray-600 sm:text-base dark:text-white">12%</div>
                </div>
            ...
            <div className="relative w-full h-full col-span-3 row-span-1 p-2 border-gray-400" data-tooltip-id="tooltipFilter" data-tooltip-content=" Filter by agent, date, queue." data-tooltip-place="bottom">
                <Filter onApplyFilters={handleApplyFilters} />
            </div>
            <Tooltip id="tooltipFilter" className="custom-tooltip" />

            <div className="col-span-3 col-start-10 row-span-3 card bg-tertiary dark:bg-primary" data-tooltip-id="tooltipAverageAnswerTimePerQueue" data-tooltip-content="This metric measures the average amount of time it takes for calls to be answered in a specific queue. A low average answer time indicates that calls are being answered quickly, while a high average answer time indicates that callers are waiting a long time to speak with an agent." data-tooltip-place="bottom">
            ...
            <div className="col-span-5 row-span-2 card bg-tertiary dark:bg-primary" data-tooltip-id="tooltipAverageQueueAbandonTime" data-tooltip-content="This metric measures the average amount of time callers wait in a queue before abandoning the call. A high average queue abandon time indicates that callers are waiting a long time before abandoning the call." data-tooltip-place="right">
            ...
            <div className="col-span-4 row-span-2 card bg-tertiary dark:bg-primary" data-tooltip-id="tooltipServiceLevel" data-tooltip-content="Service Level is the percentage of calls that are answered within a specified time threshold. Measures the efficiency of call handling by agents. A high service level indicates that calls are being answered quickly." data-tooltip-place="right">
            ...
            <div className="col-span-4 row-span-2 card bg-tertiary dark:bg-primary" data-tooltip-id="tooltipAverageContactDuration" data-tooltip-content="Average Contact Duration measures the average amount of time agents spend on a call with a customer. A low average contact duration indicates that agents are resolving customer issues quickly, while a high average contact duration indicates that agents are spending a long time on calls." data-tooltip-place="right">
            ...
            <div className="col-span-4 row-span-2 card bg-tertiary dark:bg-primary" data-tooltip-id="tooltipContactsHandled" data-tooltip-content="Contacts Handled is the total number of contacts that agents have handled within a specified time period. This metric provides insight into the volume of calls that agents are handling and can help identify trends in call volume and agent performance." data-tooltip-place="right">
                <div className="flex flex-row justify-between pb-5 mt-2">
