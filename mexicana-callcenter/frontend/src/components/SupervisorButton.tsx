...
const SupervisorButton: React.FC<OverviewButtonProps> = ({ handleClick }) => {
  ...
  const buttonSubtitleSizeClass = 'text-sm md:text-base';
    ...
    return (
      <button className={`relative flex items-center justify-center w-full ${paddingClass} mt-3 text-white transition bg-gradient-to-r from-customGreen to-primary ease-in-out duration-300 rounded-xl  hover:opacity-75 border-2 border-gray-400`}
      onClick={handleClick}>
